"""Post-processing LLM-generated Python code implemented using tree-sitter."""

import os
import pathlib
from typing import Dict, Generator, List, Optional, Set, Tuple

import tree_sitter_python
from tqdm import tqdm
from tree_sitter import Language, Node, Parser

from evalplus.data import (
    get_human_eval_plus,
    get_mbpp_plus,
    load_solutions,
    write_directory,
    write_jsonl,
)

from evalplus.syncheck import syntax_check

CLASS_TYPE = "class_definition"
FUNCTION_TYPE = "function_definition"
IMPORT_TYPE = ["import_statement", "import_from_statement"]
IDENTIFIER_TYPE = "identifier"
ATTRIBUTE_TYPE = "attribute"
RETURN_TYPE = "return_statement"
EXPRESSION_TYPE = "expression_statement"
ASSIGNMENT_TYPE = "assignment"


def code_extract(text: str) -> str:
    """
    从给定文本中，找出能通过 syntax_check 的最长代码片段并返回。
    """
    lines = text.split("\n")
    longest_line_pair = (0, 0)
    longest_so_far = 0

    for i in range(len(lines)):
        for j in range(i + 1, len(lines)):
            current_lines = "\n".join(lines[i : j + 1])
            if syntax_check(current_lines):
                current_length = sum(1 for line in lines[i : j + 1] if line.strip())
                if current_length > longest_so_far:
                    longest_so_far = current_length
                    longest_line_pair = (i, j)

    return "\n".join(lines[longest_line_pair[0] : longest_line_pair[1] + 1])


def get_deps(nodes: List[Tuple[str, Node]]) -> Dict[str, Set[str]]:
    """
    收集每个定义(类/函数/变量)里出现的所有标识符依赖，
    返回一个 name->(identifiers) 的映射，用于做 BFS。
    """
    def dfs_get_deps(node: Node, deps: Set[str]) -> None:
        for child in node.children:
            if child.type == IDENTIFIER_TYPE:
                deps.add(child.text.decode("utf8"))
            else:
                dfs_get_deps(child, deps)

    name2deps = {}
    for name, node in nodes:
        deps = set()
        dfs_get_deps(node, deps)
        name2deps[name] = deps
    return name2deps


def get_function_dependency(entrypoint: str, call_graph: Dict[str, Set[str]]) -> Set[str]:
    """
    基于 BFS，给定 entrypoint 和（定义名->依赖集合）的 call_graph，
    找出和 entrypoint 关联的所有可达定义名称，返回它们的集合。
    """
    queue = [entrypoint]
    visited = {entrypoint}
    while queue:
        current = queue.pop(0)
        if current not in call_graph:
            continue
        for neighbour in call_graph[current]:
            if neighbour not in visited:
                visited.add(neighbour)
                queue.append(neighbour)
    return visited


def get_definition_name(node: Node) -> str:
    """
    从一个 class_definition 或 function_definition AST 节点里，获取定义名。
    如果没有找到 identifier，就返回 None。
    """
    for child in node.children:
        if child.type == IDENTIFIER_TYPE:
            return child.text.decode("utf8")
    return None


def traverse_tree(node: Node) -> Generator[Node, None, None]:
    """
    深度遍历一个节点，yield 遇到的所有子节点。
    """
    cursor = node.walk()
    depth = 0
    visited_children = False

    while True:
        if not visited_children:
            yield cursor.node
            if not cursor.goto_first_child():
                depth += 1
                visited_children = True
        elif cursor.goto_next_sibling():
            visited_children = False
        elif not cursor.goto_parent() or depth == 0:
            break
        else:
            depth -= 1


def has_return_statement(node: Node) -> bool:
    """
    检查给定的 function_definition 节点里，是否至少包含一个 return 语句。
    """
    for subnode in traverse_tree(node):
        if subnode.type == RETURN_TYPE:
            return True
    return False


def class_has_function_recursive(class_node: Node, func_name: str) -> bool:
    """
    递归 / 深度遍历 class_node，判断是否存在名为 func_name 的函数定义。
    注意: 这可以处理多层嵌套，但通常只要检查当前类的子节点就够。
    """
    for subnode in traverse_tree(class_node):
        if subnode.type == FUNCTION_TYPE:
            subname = get_definition_name(subnode)
            if subname == func_name:
                return True
    return False


def extract_target_code_or_empty(code: str, entrypoint: Optional[str] = None) -> str:
    """
    从给定代码中，抽取与 entrypoint 相关的定义(类/函数/变量)。
    这里修改了逻辑：如果 entrypoint 在某个类的内部方法里，
    就把 entrypoint 改为那个类名，从而保留该类整体。
    """
    # 1) 先用 code_extract 找到可能最完整的可解析代码
    code = code_extract(code)
    code_bytes = code.encode("utf8")

    # 2) 用 tree_sitter_python 解析 AST
    parser = Parser(Language(tree_sitter_python.language()))
    tree = parser.parse(code_bytes)
    root_node = tree.root_node

    # 3) 分类收集顶层 import, class, function, variable
    import_nodes = []
    definition_nodes = []  # [(name, node), ...]

    class_names = set()
    function_names = set()
    variable_names = set()

    for child in root_node.children:
        if child.type in IMPORT_TYPE:
            # 顶层 import / from import
            import_nodes.append(child)
        elif child.type == CLASS_TYPE:
            # 顶层 class
            class_name = get_definition_name(child)
            if class_name and class_name not in (class_names | variable_names | function_names):
                definition_nodes.append((class_name, child))
                class_names.add(class_name)
        elif child.type == FUNCTION_TYPE:
            # 顶层 function
            func_name = get_definition_name(child)
            if (
                func_name
                and func_name not in (function_names | variable_names | class_names)
                and has_return_statement(child)
            ):
                definition_nodes.append((func_name, child))
                function_names.add(func_name)
        elif child.type == EXPRESSION_TYPE and child.children[0].type == ASSIGNMENT_TYPE:
            # 顶层赋值
            subchild = child.children[0]
            var_name = get_definition_name(subchild)
            if (
                var_name
                and var_name not in (variable_names | function_names | class_names)
            ):
                definition_nodes.append((var_name, subchild))
                variable_names.add(var_name)

    # 4) 若指定了 entrypoint，则看它是否属于某个 class 的方法
    if entrypoint:
        # 4.1) 检查所有顶层定义里，是否有函数就叫 entrypoint（如果有就保持原样）
        top_level_func_names = {name for name, _ in definition_nodes if name in function_names}
        if entrypoint not in top_level_func_names:
            # 4.2) 不在顶层函数中，则去找哪个 class 内部有这个方法
            for class_name, node in definition_nodes:
                if class_name in class_names:
                    # 深度遍历 class，看看是否存在名为 entrypoint 的 function
                    if class_has_function_recursive(node, entrypoint):
                        # 找到后，把 entrypoint 替换成该 class 的名字
                        entrypoint = class_name
                        break

        # 4.3) 根据新的 entrypoint 做 BFS 依赖分析
        name2deps = get_deps(definition_nodes)
        reacheable = get_function_dependency(entrypoint, name2deps)

    # 5) 拼装输出：先输出所有 import，然后输出 BFS 可达的定义节点
    sanitized_output = b""
    for node in import_nodes:
        sanitized_output += code_bytes[node.start_byte : node.end_byte] + b"\n"

    if entrypoint:
        # 有 BFS 过滤
        for name, node in definition_nodes:
            if name in reacheable:
                sanitized_output += code_bytes[node.start_byte : node.end_byte] + b"\n"
    else:
        # 没有 entrypoint，不做过滤，全输出
        for name, node in definition_nodes:
            sanitized_output += code_bytes[node.start_byte : node.end_byte] + b"\n"

    return sanitized_output.rstrip(b"\n").decode("utf8")


def sanitize(code: str, entrypoint: Optional[str] = None) -> str:
    """
    上层封装：先尝试抽取目标代码，如果抽取结果为空，则回退到原 code_extract。
    """
    sanitized_code = extract_target_code_or_empty(code, entrypoint).strip()
    if not sanitized_code:
        return code_extract(code)
    return sanitized_code


def script(
    samples: str, inplace: bool = False, debug_task: str = None, mbpp_version="default"
):
    # task_id -> entry_point
    entry_point = {}
    # merge two datasets
    dataset = {**get_human_eval_plus(), **get_mbpp_plus(version=mbpp_version)}

    for task_id, problem in dataset.items():
        entry_point[task_id] = problem["entry_point"]

    # make a new folder with "-sanitized" suffix
    is_folder = os.path.isdir(samples)
    target_path = pathlib.Path(samples)
    if not inplace:
        if is_folder:
            new_name = target_path.name + "-sanitized"
        else:
            new_name = target_path.name.replace("deepseek_coder.jsonl", "-sanitized.jsonl")
        target_path = target_path.parent / new_name
    target_path = str(target_path)

    nsan = 0
    ntotal = 0

    new_solutions = []

    for solution in tqdm(load_solutions(samples)):
        task_id = solution["task_id"]
        if task_id not in dataset:
            print(
                f"Skiping {task_id} as it does not exist in the latest EvalPlus dataset."
            )
            continue

        function_name = entry_point[task_id] if task_id in entry_point else None
        dbg_identifier = solution["_identifier"]
        if debug_task is not None and task_id != debug_task:
            continue

        ntotal += 1
        if "solution" in solution:
            old_code = solution["solution"]
        else:
            assert "completion" in solution
            old_code = dataset[task_id]["disprompt"] + "\n" + solution["completion"]

        new_code = sanitize(code=old_code, entrypoint=function_name)

        # if changed, print the message
        if new_code != old_code:
            msg = "Sanitized: " + dbg_identifier
            if is_folder:
                msg += " -> " + dbg_identifier.replace(samples, target_path)
            print(msg)
            nsan += 1

        new_solutions.append({"task_id": task_id, "solution": new_code})

    if is_folder:
        write_directory(target_path, new_solutions)
    else:
        write_jsonl(target_path, new_solutions)

    if nsan > 0:
        print(f"Sanitized {nsan} out of {ntotal} files.")
    else:
        print(f"All files seem valid -- no files were sanitized.")
    print(f"Check the sanitized files at {target_path}")


def main():
    from fire import Fire
    Fire(script)


if __name__ == "__main__":
    main()
code = """
from typing import List

class Solution(object):
    def spiralOrder(self, matrix: List[List[int]]) -> List[int]:
        if not matrix or not matrix[0]:
            return []
        
        result = []
        top, bottom, left, right = 0, len(matrix) - 1, 0, len(matrix[0]) - 1
        
        while top <= bottom and left <= right:
            # Traverse from left to right
            for i in range(left, right + 1):
                result.append(matrix[top][i])
            top += 1
            
            # Traverse from top to bottom
            for i in range(top, bottom + 1):
                result.append(matrix[i][right])
            right -= 1
            
            if top <= bottom:
                # Traverse from right to left
                for i in range(right, left - 1, -1):
                    result.append(matrix[bottom][i])
                bottom -= 1
            
            if left <= right:
                # Traverse from bottom to top
                for i in range(bottom, top - 1, -1):
                    result.append(matrix[i][left])
                left += 1
        
        return result
"""
entry_point = 'spiralOrder'
    # sanitize(code)
...
def get_dataset_metadata(name: str, version: str, mini: bool, noextreme: bool = False):
    ...
    if noextreme:
        extra = "-NoExtreme"
    url = f"https://github.com/evalplus/{name.lower()}_release/releases/download/{version}/{name}{extra}deepseek_coder.jsonl.gz"
    cache_path = os.path.join(CACHE_DIR, f"{name}{extra}-{version}deepseek_coder.jsonl")
...
def load_solutions(sample_path: PathLike) -> Iterable[Dict]:
    ...
    Different from `sample.jsonl`, the solutions must be complete (with disprompt prefix).
    """
